...
(function (root, window, document, factory, undefined) {
...
}(this, window, document, (window, document) => {
  ...
  function initialise(containerSelector, options) {
    ...
    function onMouseEnterOrLeave(e) {
      ...
      normalSelectors.forEach((normalSelector) => {
    ...
    function prepareDom() {
      ...
      if (options.fixedElements && options.css3) {
        $(options.fixedElements).forEach((item) => {
    ...
    function enableYoutubeAPI() {
      $('iframe[src*="youtube.com/embed/"]', container).forEach((item) => {
    ...
    function getDestinationPosition(element) {
      ...
      const { bigSectionsDestination } = options;
    ...
    function lazyLoadOthers() {
      ...
      $(`${SECTION_SEL}:not(${ACTIVE_SEL})`).forEach((section) => {
    ...
    function lazyLoad(destiny) {
      ...
      $('img[data-src], img[data-srcset], source[data-src], source[data-srcset], video[data-src], audio[data-src], iframe[data-src]', panel).forEach((element) => {
    ...
    function playMedia(destiny) {
      ...
      $('video, audio', panel).forEach((element) => {
      ...
      // youtube videos
      $('iframe[src*="youtube.com/embed/"]', panel).forEach((element) => {
    ...
    function getFocusables(el) {
      return [].slice.call($(focusableElementsString, el)).filter((item) => item.getAttribute('tabindex') !== '-1'
    ...
    function setIsScrollAllowed(value, direction, type) {
      ...
      // all directions?
        ...
        Object.keys(isScrollAllowed[type]).forEach((key) => {
    ...
    function destroy(all) {
      ...
      ['mouseenter', 'touchstart', 'mouseleave', 'touchend'].forEach((eventName) => {
    ...
    function destroyStructure() {
      ...
      $('img[data-src], source[data-src], audio[data-src], iframe[data-src]', container).forEach((item) => {
        setSrc(item, 'src');
      });

      $('img[data-srcset]').forEach((item) => {
        setSrc(item, 'srcset');
      });
      ...
      $body.className.split(/\s+/).forEach((className) => {
      ...
      $(`${SECTION_SEL}, ${SLIDE_SEL}`).forEach((item) => {
      ...
      // Unwrapping content
      [TABLE_CELL_SEL, SLIDES_CONTAINER_SEL, SLIDES_WRAPPER_SEL].forEach((selector) => {
        $(selector, container).forEach((item) => {
    ...
    function displayWarnings() {
      ...
      options.anchors.forEach((name) => {
        // case insensitive selectors (http://stackoverflow.com/a/19465187/1081396)
        const nameAttr = [].slice.call($('[name]')).filter((item) => item.getAttribute('name') && item.getAttribute('name').toLowerCase() == name.toLowerCase());

        const idAttr = [].slice.call($('[id]')).filter((item) => item.getAttribute('id') && item.getAttribute('id').toLowerCase() == name.toLowerCase());
